# Netflix Clone

JavaScript full stack application developed with the goal of demonstrating knowledge.
This repo contain the frontend application developed with react.

###  Development Process
First I created the project's layout with html and sass. If you want to see the code, you will can do it [in this repo](https://github.com/ivansevillaa/Netflix-Clone-Layout "in this repo").  

The next step will be to make the API with node.js and express, you will see the process [in this repo](https://github.com/ivansevillaa/Netflix-Clone-API "in this repo").

...

### Tech Stack
#### For the layout:  
  <img src="https://cdn1.iconfinder.com/data/icons/logotypes/32/badge-html-5-512.png" width="50" height="50"><img src="https://www.shareicon.net/data/256x256/2016/07/10/119876_css_512x512.png" width="60" height="60">
 

### Features
...

### Dowload and Use
...

### License
[MIT](https://opensource.org/licenses/MIT "MIT")

| TODO: |
| :--- |
| Frontend  |
| SSR |
| Backend |
| Authentication |
| Backend for Frontend |
| Tests |
| Deployment |
